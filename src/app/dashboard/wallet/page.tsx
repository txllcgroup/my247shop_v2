...
export default function WalletPage() {
  ...
  const handleWithdraw = async () => {
    ...
    const threshold = currency === 'NGN' ? 2000 : 100;

    if (isNaN(withdrawalAmount) || withdrawalAmount < threshold) {
      alert(`Minimum payout amount is ${currency === 'NGN' ? '₦2,000' : '$100'}.`);
      return;
    }

    // Payout implementation would go here once the endpoint is ready
    alert(`Withdrawal request of ${currency} ${withdrawalAmount} submitted for processing.`);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 px-4 lg:px-0">
        ...
        <div className="bg-white border-2 border-gray-200 rounded-3xl p-8 lg:p-12 flex flex-col justify-between aspect-auto md:aspect-[4/3] relative overflow-hidden group shadow-sm">
          ...
          <div className="relative z-10 mt-12 md:mt-0">
              ...
              <div className="space-y-4 animate-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 bg-black text-white px-8 py-4 rounded-2xl text-lg font-semibold hover:bg-gray-800 transition-colors shadow-lg"
                  >
                    Confirm Withdraw
                  </button>
                </div>
            ...
            <div className="mt-6 flex items-start gap-3 bg-blue-50 border-2 border-blue-100 p-4 rounded-2xl">
              <svg className="w-5 h-5 text-blue-600 mt-1 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              <p className="text-sm text-blue-800 font-bold leading-snug italic">
                Minimum threshold: ₦2,000 (Naira) or $50 (USD) for payout.
              </p>
            </div>
          </div>
        </div>
              ...
              <tbody className="divide-y-2 divide-gray-100">
                {transactions.map((tx) => (
                  <tr key={tx.id} className="hover:bg-gray-50 transition-colors group border-b-2 border-gray-100 last:border-0 relative cursor-pointer">
                    ...
                    <td className="px-6 py-5">
                      <span className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide border-2 inline-flex items-center gap-1.5 whitespace-nowrap ${tx.status === 'Completed' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
