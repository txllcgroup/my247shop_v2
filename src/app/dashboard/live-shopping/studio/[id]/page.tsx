...
export default function LiveStudio() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const sessionData = await LiveShoppingService.getSessions();
        const current = sessionData.find(s => s.id === id);
        if (current) {
          setSession(current);
          setIsLive(current.status === 'Live');
          setSelectedProductId(current.featuredProductId || null);
        }

        // Fetch products using ApiClient for proper auth headers
        const productData = await ApiClient.get('/products');
  ...
  const setupSignalR = async () => {
    // Determine the base URL dynamically from ApiClient or default
    const baseUrl = (ApiClient as any).defaults?.baseURL || "https://my247v2.airshop247.com/api";
  ...
  const featureProduct = async (productId: string) => {
    try {
      // Toggle off if clicking the same product
      const newProductId = selectedProductId === productId ? "" : productId;
      await LiveShoppingService.featureProduct(id as string, newProductId);
      setSelectedProductId(newProductId || null);
      await hubConnection.current?.invoke("FeatureProduct", id, newProductId);
    } catch (err) {
  ...
  const featuredProduct = products.find(p => p.id === selectedProductId);
          ...
          <div className="absolute bottom-10 left-10 animate-in slide-in-from-left duration-500">
            <div className="bg-white rounded-3xl p-4 shadow-2xl flex items-center gap-4 border-2 border-black">
              ...
              <div className="pr-4">
                ...
                <p className="font-bold text-gray-500">₦{featuredProduct.price.toLocaleString()}</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="w-full md:w-[400px] bg-white flex flex-col shadow-2xl z-40">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3">
            ...
            {products.length === 0 && <p className="text-gray-400 italic text-center py-10">No products available</p>}
          </div>
        </div>
