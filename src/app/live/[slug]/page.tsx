...
export default function LiveViewer() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 1. Get Store Details
        const storeRes = await fetch(`https://my247v2.airshop247.com/api/storefront/store/${slug}`);
        const storeData = await storeRes.json();
        setStore(storeData);

        // 2. Get Active Live Session
        const liveRes = await fetch(`https://my247v2.airshop247.com/api/storefront/store/${storeData.id}/live`);
        const liveData = await liveRes.json();
        setSession(liveData);

        if (liveData) {
            setupSignalR(liveData.id);
            if (liveData.featuredProductId) {
                fetchProduct(storeData.id, liveData.featuredProductId);
            }
        }
      } catch (err) {
        console.error(err);
  ...
  const setupSignalR = async (sessionId: string) => {
    ...
    connection.on("ProductFeatured", (productId) => {
      fetchProduct(store.id, productId);
    });
  ...
  const fetchProduct = async (storeId: string, productId: string) => {
    try {
      const res = await fetch(`https://my247v2.airshop247.com/api/storefront/store/${storeId}/products`);
  ...
  if (!session || session.status !== 'Live') return (
    <div className="fixed inset-0 bg-neutral-900 flex flex-col items-center justify-center text-center p-8">
        ...
        <p className="text-white/60">Follow {store?.name} to get notified when they go live!</p>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 bg-black overflow-hidden font-sans">
      ...
      <div className="absolute top-0 left-0 right-0 p-6 flex items-start justify-between bg-gradient-to-b from-black/60 to-transparent z-20">
         ...
         <button className="bg-white/10 backdrop-blur-md text-white w-10 h-10 rounded-full flex items-center justify-center hover:bg-white/20 transition-all">
